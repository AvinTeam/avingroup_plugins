<div class="form-field">
    <label for="service_icon">آیکون خدمت</label>
    <input type="hidden" name="service_icon" id="service_icon" value="" />
    <div id="service_icon_preview" style="margin: 10px 0; display: none;">
        <img src="" style="max-width: 50px; height: 50px; object-fit: contain;" />
    </div>
    <input type="button" class="button button-secondary" id="upload_service_icon"
        value="آپلود آیکون" />
    <input type="button" class="button button-secondary" id="remove_service_icon"
        value="حذف آیکون" style="display: none;" />
    <p class="description">آیکون خدمت را انتخاب کنید</p>
</div>

<div class="form-field">
    <label for="service_poster">پستر خدمت</label>
    <input type="hidden" name="service_poster" id="service_poster" value="" />
    <div id="service_poster_preview" style="margin: 10px 0; display: none;">
        <img src="" style="max-width: 200px; height: auto;" />
    </div>
    <input type="button" class="button button-secondary" id="upload_service_poster"
        value="آپلود پستر" />
    <input type="button" class="button button-secondary" id="remove_service_poster"
        value="حذف پستر" style="display: none;" />
    <p class="description">پستر خدمت را انتخاب کنید</p>
</div>
